...
# Configure logging
logging.basicConfig(filename="logs/pacs008_generation.log", level=logging.INFO,
...
def generate_pacs008(is_unusual=False):
    """
    Generates a highly realistic pacs.008 XML message with:
    - Real BIC codes
    - Real addresses
    ...
    if is_unusual or is_sanctioned:
        ...
        if reason and reason != "USUAL":  # Only log actual anomalies
            logging.warning(f"Unusual pacs.008 Transaction Detected - Reason: {reason}")
...
def save_pacs008_messages(num_messages=100):
    """
    Generates and saves highly realistic pacs.008 messages with:
    - Real banks
    - Real addresses
    - Cross-border logic
    ...
    for i in range(1, usual_count + 1):
        message = generate_pacs008(False)
        save_message(message, "pacs008", i, False, "xml")

    for i in range(usual_count + 1, num_messages + 1):
        message = generate_pacs008(True)
        save_message(message, "pacs008", i, True, "xml")
...
def save_message(message, folder, index, is_unusual, extension="xml"):
    """
    Saves a pacs.008 message to a file with 'usual' or 'unusual' in the name.
    """
    try:
